...
private:
    ...
    QColor m_sunken = "#212c30";          // Фон утопленных элементов (поля ввода, фон слайдеров и чекбоксов и т.д)
    QColor m_text = "#ffffff";            // Текст (иконки и другие элементы на фоне окна, утопленных и возвышенных элементов)
    ...
    QColor m_controlBorder = "#6c8693";   // Фон и обводка мелких управляющих элементов (хендлы, переключатели)
...
class INDUSTRIAL_CONTROLS_EXPORT IndustrialTheme : public QObject
{
    ...
    Q_PROPERTY(bool scrollInteractive READ scrollInteractive WRITE setScrollInteractive NOTIFY scrollInteractiveChanged)
    Q_PROPERTY(qreal animationTime READ animationTime WRITE setAnimationTime NOTIFY animationTimeChanged)

    Q_PROPERTY(IndustrialThemeColors dayColors READ dayColors WRITE setDayColors NOTIFY dayColorsChanged)
    Q_PROPERTY(IndustrialThemeColors nightColors READ nightColors WRITE setNightColors NOTIFY nightColorsChanged)
